...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  int playAdventurer() {
        ...
        //***BUG BELOW: 1 changed to 0***
        cardDrawn = state->hand[currentPlayer][state->handCount[currentPlayer]-0];//top card of hand is most recently drawn card.
  ...
  int playSmithy() {
          //+3 Cards ***BUG BELOW: 3 changed to 100***
          for (i = 0; i < 100; i++)
    	{
    	  drawCard(currentPlayer, state);
    	}
  ...
  int playSeahag() {
    ...
    if (i != currentPlayer){
      ...
      //***BUG BELOW: changed = curse to = gold
      state->deck[i][state->deckCount[i]--] = gold;//Top card now a curse
    }
      }
      return 0;
  }
  ...
  int playMinion() {
      ...
      else if (choice2)		//discard hand, redraw 4, other players with 5+ cards discard hand and draw 4
    {
      ...
      //draw 4 ***BUG BELOW: Changed 4 to 1000***
      for (i = 0; i < 1000; i++)
        {
          drawCard(currentPlayer, state);
        }
